...
import sys
import json
from dollar_recognizer_pro import DollarRecognizer, Point  # Import DollarRecognizer and Point
...
class LetterDrawer(QMainWindow):
    ...
    def on_gesture_recognized(self, gesture_name, score):
        ...
        if gesture_name[1] in COMMANDS["commands"]:
            print(gesture_name[1])
            available_commands = COMMANDS["commands"][gesture_name[1]]
            print(available_commands)
            n = len(available_commands)
            g = int(gesture_name[0])-1
            if g <= n:
                print(available_commands[g]["shortcut"])
                pyautogui.hotkey(*available_commands[g]["shortcut"].split("+"))
                self.close_command_menu()



    def open_command_menu(self, detected_character, x, y):
        if detected_character[1] in COMMANDS["commands"]:
            ...
            if detected_character[0] <= n:
                pyautogui.hotkey(*available_commands[detected_character[0]]["command"].split("+"))
                print(*available_commands[detected_character[0]]["command"].split("+"))
